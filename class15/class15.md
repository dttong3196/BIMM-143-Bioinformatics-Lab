...
Dont match because there is a length data, so remove the column. The
*length* column in countData is going to cause problems with the
required atching to the metadata file so lets remove it here.
...
Double Check that colnames in countData
    match
...
Access SRR. We can use the all() function to check if all entries in a
vector are TRUE
...
(i.e. rows that have 0 accross all cols).
...
```
    ...
    ## Info: Working in directory /Users/dtong/Desktop/BIMM 143 Bioinformatics/BIMM-143-GitHub/class15
